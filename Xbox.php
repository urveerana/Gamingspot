...
<div class="row">
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit']) && isset($_POST['orderby'])) {
        ...
        if ((int)$_POST['orderby'] == 1) {
            $sql= "SELECT * FROM `products` WHERE product_category = 'Xbox' ORDER BY product_name ASC";
            echo '<h3 style="text-align: center;font-weight: bold">Now Viewing Products By Name</h3>';
        } else if((int)$_POST['orderby'] == 2) {
            $sql= "SELECT * FROM `products` WHERE product_category = 'Xbox' ORDER BY product_price+0 ASC";
  ...
  "  alt="<?php echo $row['product_name'] ?>">
        ...
        <?php
        $query = "SELECT * FROM `products` WHERE product_category = 'Xbox' ";
        $result = mysqli_query($mysqli, $query);
        ?>    <h3 style="text-align: center;font-weight: bold">All Products In XBOX SERIES S</h3>
        <?php
