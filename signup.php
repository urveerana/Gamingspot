...
function checkUser(){
    ...
    if(empty(trim($_POST["user"]))){
        ...
        if($stmt = $mysqli->prepare($sql)){
            $stmt->bind_param("s", $parameter_username);
...
function checkPassword(){
    ...
    if(empty(trim($_POST["password"]))){
        ...
        $password_error = "Password should contain 8 value";
    } else{
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($user_err) && empty($email_error) && empty($password_error) && empty($confirm_password_error)){
        ...
        if($stmt = $mysqli->prepare($sql)){
            $stmt->bind_param("sss", $parameter_username, $parameter_email, $parameter_password);
...
<div class="wrapper">
    ...
    <form method="POST"
        ...
        <div class="form-group" style="" ><input type="submit" id="submit" class="btn btn-primary" style="background-color: green;width: 25%;color: white;margin: 0px -10px" value="Submit">
            <input type="reset" id="cancel" class="btn btn-primary" style="background-color: red;width: 25%;color: white;margin: 0px 10px" value="Cancel">
        </div>

    </form>
    <br>
    <div class="form-group" style="" ><label style="padding: 8px"><b>Already Registered?</b></label><a href="login.php"><input type="submit" id="submit"  class="btn btn-primary" style="background-color:margin: 0px 90px; darkblue;width: 25%;color: white" value="Login">
        </a>
    </div>
</div>
</body>
